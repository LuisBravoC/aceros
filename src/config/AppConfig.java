...
public final class AppConfig {
    ...
    static {
        // Allow test runs to override config file via -Dapp.config=config/test.config.properties
        String configPath = System.getProperty("app.config", "config/config.properties");
        // Try loading from workspace-relative path first
        try (InputStream in = new FileInputStream(configPath)) {
            PROPS.load(in);
            LOGGER.log(Level.CONFIG, "Loaded config from {0}", configPath);
